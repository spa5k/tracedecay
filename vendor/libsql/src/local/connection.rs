...
impl Connection {
    ...
    ///
    /// TRACEDECAY PATCH (tursodatabase/libsql#2251): `disconnect` must be
    /// idempotent. `LibsqlConnection`'s `Drop` calls it, then the inner
    /// `Connection`'s `Drop` calls it again on the same instance; the
    /// `Arc::get_mut(drop_ref)` guard only checks unique ownership (true
    /// both times), so the second call re-closed the already-freed
    /// `sqlite3` handle. On Windows that use-after-free intermittently
    /// faults with `STATUS_ACCESS_VIOLATION` (0xc0000005) at connection
    /// teardown. Null the handle after the first close so later calls are
    /// no-ops.
