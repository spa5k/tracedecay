...
fn json_rpc_response(stdout: &[u8], id: i64) -> Value {
    let stdout_text = String::from_utf8(stdout.to_vec()).unwrap();
    stdout_text
        .lines()
        .filter_map(|line| serde_json::from_str::<Value>(line).ok())
...
/// Runs `serve` over stdio with an initialize + one tools/call, returning the
/// process output once stdin closes.
fn run_serve_initialize_and_tool_call(home: &Path, explicit_path: &Path, cwd: &Path) -> Output {
...
/// A dead MCP process permanently kills the client scope (Cursor never
/// retries a failed spawn), so an explicit uninitialized `--path` must NOT
/// exit: serve completes the handshake and answers tool calls with an
/// actionable error naming the explicit path — and still never silently
/// serves the registered global-fallback project.
#[tokio::test]
async fn explicit_uninitialized_path_serves_degraded_error_instead_of_global_fallback() {
    ...
    let output = run_serve_initialize_and_tool_call(home.path(), explicit.path(), explicit.path());
...
/// Degraded serve must recover in-session: once `tracedecay init` makes
/// project resolution succeed, the very next tool call is served for real —
/// no server toggle or window reload (which Cursor would otherwise require).
#[cfg(unix)]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn degraded_serve_recovers_after_project_init() {
    ...
    // Reads lines until the JSON-RPC response with `id` appears (skipping
    // notifications and warning lines the recovered server may interleave).
    ...
    assert_eq!(
        ...
        json!(true),
        "pre-init tool call should return the degraded error:\n{degraded}"
    );
    ...
    assert!(
        ...
        "post-init tool call should be served by the recovered server:\n{recovered}"
    );
    ...
    assert_eq!(
        ...
        "recovered server must serve the originally requested project"
    );
...
async fn same_depth_descendant_global_fallback_is_ambiguous() {
    ...
    assert_eq!(
        tool_call["result"]["isError"],
        json!(true),
        "ambiguity must not select an arbitrary project:\n{tool_call}"
    );
    let text = tool_call["result"]["content"][0]["text"]
        .as_str()
        .expect("degraded tool error should carry text content");
