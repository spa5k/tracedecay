...
fn tokensave_command(home: &std::path::Path, project: &std::path::Path) -> Command {
...
fn run_with_timeout(mut command: Command, timeout: Duration) -> std::process::Output {
    ...
    loop {
        ...
        assert!(
            ...
            "tokensave hung with stdin closed after {:?}",
            started.elapsed()
        );
...
fn init_skips_gitignore_prompt_when_stdin_not_a_terminal() {
    ...
    let mut command = tokensave_command(home.path(), project.path());
    command.arg("init");
    ...
    assert!(
        project.path().join(".tokensave").is_dir(),
    ...
    assert!(
        stderr.contains("Non-interactive: skipped adding .tokensave to .gitignore"),
...
fn bare_invocation_skips_create_prompt_when_stdin_not_a_terminal() {
    ...
    let output = run_with_timeout(
        tokensave_command(home.path(), project.path()),
        Duration::from_secs(30),
    );

    assert!(
        output.status.success(),
        "bare tokensave should exit cleanly non-interactively\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    assert!(
