...
All Cursor hooks are fail-open and only act when a `.tokensave/` index already exists. The `subagentStart` and `preToolUse` hooks are pure **soft hints**: they only ever inject nonblocking `additional_context` and never deny, rewrite, or block a tool call, so they cannot stall the Agent even if the index is missing or the hint logic misfires. Hints are conservative (high-confidence patterns only) and deduped to one per category per session to avoid noise. **Blind spot:** Cursor hooks only observe the Cursor Agent's own actions and IDE lifecycle. Manual/external-terminal `git checkout` and in-place branch switches are NOT seen by these hooks (`workspaceOpen` does not fire for an in-place checkout). For those, the git post-commit hook and the on-demand MCP staleness check remain the freshness mechanism.
...
All Codex hooks are fail-open and only act when a `.tokensave/` index exists. The `SubagentStart` and `PreToolUse` hooks are pure soft hints — they only inject `additionalContext` and never deny or rewrite tool calls. **Trust gate:** Codex skips new or changed non-managed command hooks until you trust them — run `/hooks` inside Codex to review and trust the tokensave hooks (the installer prints this reminder; `--dangerously-bypass-hook-trust` exists for one-off non-interactive runs). **Blind spots:** `PostToolUse` only fires for `apply_patch` edits and "simple" Bash — raw-shell file edits, `unified_exec`, and `WebSearch` are not observed; there is no first-class branch-switch event, so branch switches are derived from Bash `git` commands. The `PreToolUse` soft-hint hook shares this limited visibility (it cannot see `unified_exec`/`WebSearch`/raw-shell edits), but because it only ever adds context and never blocks, missing those events simply means no hint rather than an incorrect denial.
...
> **Note on the `.cursor/` and `.codex/` configs in this repository.** These are generated installer output, committed deliberately so the tokensave repo dogfoods its own Cursor/Codex integration. They are produced by `tokensave install --local --agent cursor` and `tokensave install --local --agent codex` — regenerate them with those commands rather than hand-editing. Because the generated hook commands embed the resolved absolute `tokensave` binary path, the committed files contain machine-local debug paths; that is expected for this repo's checked-in copies.
