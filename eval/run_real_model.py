#!/usr/bin/env python3
"""Cost-gated real-model layer of the tokensave memory-hygiene eval suite.

Seeds a throwaway fixture project, points a REAL agent (Hermes by default,
optionally `cursor-agent`) at it through the generated tokensave plugin /
MCP server, sends the scenario prompts, then asserts on end-state with plain
SQL against the fixture's `.tokensave/tokensave.db`.
...
DEFAULT_PROFILE = "tokensave-eval"
...
def parse_args(argv):
    ...
    parser.add_argument(
        "--tokensave-bin",
        type=Path,
        help="tokensave binary (default: target/debug/tokensave if built, else PATH).",
    )
    parser.add_argument(
        "--keep-fixture",
        action="store_true",
...
def resolve_tokensave_bin(explicit):
    if explicit:
        return str(explicit)
    debug_bin = REPO_ROOT / "target/debug/tokensave"
    if debug_bin.exists():
        return str(debug_bin)
    found = shutil.which("tokensave")
    if not found:
        sys.exit("no tokensave binary: build target/debug or install one on PATH")
    return found
...
def build_fixture(scenario, tokensave_bin):
    fixture = Path(tempfile.mkdtemp(prefix=f"tokensave-eval-{scenario['id']}-"))
    (fixture / "src").mkdir()
    ...
    run([tokensave_bin, "init"], cwd=fixture, timeout=300)
    for fact in scenario["setup"].get("facts", []):
        args = json.dumps(
            {"action": "add", "content": fact["content"], "category": fact["category"]}
        )
        run([tokensave_bin, "tool", "fact_store", "--args", args], cwd=fixture, timeout=120)
    db = sqlite3.connect(fixture / ".tokensave/tokensave.db")
    with db:
        for fact in scenario["setup"].get("facts", []):
            db.execute(
...
def drive_cursor_agent(args, scenario, fixture, log_dir):
    ...
    run(
        [args.tokensave_bin, "install", "--agent", "cursor", "--local"],
        cwd=fixture,
        timeout=120,
    )
    transcripts = []
...
def main(argv):
    ...
    args.tokensave_bin = resolve_tokensave_bin(args.tokensave_bin)
    ...
    report = {
        ...
        "driver": args.driver,
        "model": args.model,
        "tokensave_bin": str(args.tokensave_bin),
        "scenarios": [],
    }
