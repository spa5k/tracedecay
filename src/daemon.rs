...
impl ServiceRunner {
    ...
    fn log_hint(&self) -> String {
        match self {
            #[cfg(target_os = "linux")]
            Self::Systemd => format!("journalctl --user -u {SERVICE_NAME} -f"),
            #[cfg(target_os = "macos")]
...
#[cfg(any(test, target_os = "macos"))]
fn launchd_install_command_args(
    domain: &str,
    target: &str,
    service_path: &Path,
) -> Vec<Vec<String>> {
...
#[cfg(any(test, target_os = "macos"))]
fn launchd_refresh_command_args(
    domain: &str,
    target: &str,
    service_path: &Path,
) -> Vec<Vec<String>> {
...
#[cfg(any(test, target_os = "macos"))]
fn launchd_uninstall_command_args(target: &str) -> Vec<Vec<String>> {
    vec![
...
mod tests {
    ...
    fn launchd_command_args_match_install_refresh_and_uninstall_mapping() {
        ...
        assert_eq!(
            super::launchd_install_command_args(
                "gui/501",
        ...
        assert_eq!(
            super::launchd_refresh_command_args(
                "gui/501",
                "gui/501/com.tracedecay.daemon",
                &service_path
            )
        ...
        assert_eq!(
            super::launchd_uninstall_command_args("gui/501/com.tracedecay.daemon"),
            vec![
                vec![
                    "bootout".to_string(),
                    "gui/501/com.tracedecay.daemon".to_string()
