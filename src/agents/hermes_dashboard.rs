...
//! same embedded dist data the standalone `tokensave dashboard` server
...
const BIN_PLACEHOLDER: &str = "DEPLOYED_TOKENSAVE_BIN = None";
const ROOT_PLACEHOLDER: &str = "DEPLOYED_PROJECT_ROOT = None";

/// Deploys the dashboard wrapper into `<plugin_dir>/dashboard/`.
///
/// `tokensave_bin` and `pinned_project_root` are baked into the deployed
/// `plugin_api.py` (the wrapper's `TOKENSAVE_BIN` / `TOKENSAVE_DASHBOARD_PROJECT`
/// environment variables still win at runtime), so the spawned
/// `tokensave dashboard` child serves the profile's pinned project without
...
pub(super) fn install_dashboard(
    ...
    std::fs::create_dir_all(&dist_dir).map_err(|e| TokenSaveError::Config {
    ...
    write_text_file(
        ...
        &plugin_api(tokensave_bin, pinned_project_root)?,
    )?;
...
fn manifest_json() -> Result<String> {
    let mut manifest: serde_json::Value =
        serde_json::from_str(MANIFEST_JSON).map_err(|e| TokenSaveError::Config {
...
fn plugin_api(tokensave_bin: &str, pinned_project_root: Option<&str>) -> Result<String> {
    let encode = |value: &str| {
        // JSON string literals are valid Python string literals.
        serde_json::to_string(value).map_err(|e| TokenSaveError::Config {
    ...
    if !PLUGIN_API_PY.contains(BIN_PLACEHOLDER) || !PLUGIN_API_PY.contains(ROOT_PLACEHOLDER) {
        return Err(TokenSaveError::Config {
            message: "embedded plugin_api.py is missing its deploy placeholders — \
                      this is a tokensave build bug"
                .to_string(),
        });
    }
    let mut api = PLUGIN_API_PY.replace(
        BIN_PLACEHOLDER,
        &format!("DEPLOYED_TOKENSAVE_BIN = {}", encode(tokensave_bin)?),
    );
...
mod tests {
    ...
    fn manifest_is_stamped_with_crate_version() {
        ...
        assert_eq!(parsed["provider"]["name"], "tokensave");
    }

    #[test]
    fn plugin_api_bakes_bin_and_pin() {
        let api = plugin_api("/usr/local/bin/tokensave", Some("/home/u/proj")).unwrap();
    ...
    fn plugin_api_without_pin_keeps_none_default() {
        let api = plugin_api("/usr/local/bin/tokensave", None).unwrap();
        assert!(api.contains(ROOT_PLACEHOLDER));
        assert!(!api.contains(BIN_PLACEHOLDER));
    }
    ...
    fn plugin_api_escapes_quotes_in_paths() {
        let api = plugin_api("/tmp/we\"ird/tokensave", None).unwrap();
        assert!(api.contains(r#"DEPLOYED_TOKENSAVE_BIN = "/tmp/we\"ird/tokensave""#));
    }
