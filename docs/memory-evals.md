...
tokensave's holographic memory is only useful if agents keep it clean: no run
...
1. **Fixture** — a throwaway project directory is created, `tokensave init`
   builds a real `.tokensave/` store, and the scenario's setup block seeds
...
2. **Drive** — either a scripted tool-call sequence (deterministic layer) or a
   real agent prompted over the generated tokensave integration (real-model
   layer) exercises the memory write/recall/curation paths.
3. **Assert** — end-state is checked with plain SQL against the fixture's
   `.tokensave/tokensave.db` (plus structured checks of the
   `tokensave memory curate` dry-run report for curation scenarios).
...
real `tokensave` binary — the same code path MCP tool calls hit — and runs in
...
- **Hermes** (default): the runner writes a dedicated Hermes profile
  (`~/.hermes/profiles/tokensave-eval`), runs
  `tokensave install --agent hermes --profile tokensave-eval
...
- **cursor-agent** (experimental): `tokensave install --agent cursor --local`
